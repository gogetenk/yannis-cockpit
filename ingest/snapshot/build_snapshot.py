...
def build_signals(yazio: list[dict], measurements: list[dict], activity: list[dict], hc_records: list[dict], today: date, huawei_daily: list[dict] | None = None) -> list[dict]:
    ...
    if len(sleep_pts) >= 3:
        ...
        # Weekly cumulative delta vs 7h30/nuit target.
        ...
        out.append({
            ...
            "value": f"{abs(int(round(delta_h)))} h",
            "unit": "/ semaine",
            "status": "watch" if watch else "ok",
            "status_label": "à surveiller" if watch else "conforme",
...
def build_ai_brief(payload: dict, full_context: dict | None = None) -> str | None:
    ...
    system_prompt = (
        "Tu es le coach quotidien de Yannis (35 ans, homme, 173 cm, programme "
        "perte de poids sous Wegovy). Tu génères 1 phrase par snapshot, "
        "régénérée 3x/jour. Tu sais qu'il lit ce message brièvement chaque jour.\n\n"
        "MISSION: focus sur les LOOPS COURTS (24h-7j) — sommeil, HR repos, "
        "stress, déficit calorique, protéines, activité. Ces signaux sont "
        "mesurés tous les jours via Withings/Huawei/Yazio, donc une action "
        "donnée AUJOURD'HUI peut être VÉRIFIÉE EN 1 À 7 JOURS sur le même "
        "dashboard.\n\n"
        "INTERDIT ABSOLUMENT: actions à horizon long (bilan sanguin, supplément "
        "qui se mesure à 6 mois, statine, méthylfolate, etc.). Le dashboard a une "
        "card 'Bilan biologique' dédiée pour ça — c'est pas ton job.\n\n"
        "Hiérarchie des bons outputs (du meilleur au pire):\n"
        "1. Action ce soir/cette semaine avec horizon de vérif explicite:\n"
        "   - 'HR repos est monté de 53 à 58 bpm en 5 jours, signe de fatigue: "
        "      coupe les écrans à 22h ce soir, retour sub-55 vérifiable d'ici dimanche.'\n"
        "   - 'Sommeil 6h20 moyen sur 4 nuits vs cible 7h30, couche-toi 1h plus tôt "
        "      ce soir et regarde le sommeil profond demain matin.'\n"
        "2. Corrélation 2 signaux courts: 'Stress 7j à +18 % du baseline coïncide "
        "   avec sommeil deep en baisse, journée yoga ou marche cette semaine.'\n"
        "3. Projection court terme sur le poids: 'À ce rythme la cible 83 kg sera "
        "   atteinte sous 8 jours.'\n"
        "4. SI TOUT EST CONFORME sur les loops courts: dis-le franchement en 1 "
        "   phrase, sans inventer un problème. Exemple: 'Tous les signaux sont "
        "   conformes sur la fenêtre 7 jours, continue comme ça.'\n\n"
        "À ÉVITER:\n"
        "- Recap des chiffres déjà visibles\n"
        "- Conclusions vagues ('à monitorer', 'demande surveillance')\n"
        "- Inventer chiffres, dates, doses, plans\n"
        "- Em-dash, jargon, cheerleading, guillemets autour\n"
        "- Plus de 35 mots\n\n"
